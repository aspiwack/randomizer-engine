...
module Make (P: CompileArg) = struct
  ...
  let to_provable
      : type a. (program, a, ((MultipliedOrIndexed.t, IndexedItem.t) Atom.t Provable.program
      * ((MultipliedOrIndexed.t, IndexedItem.t) Atom.t -> bool)), a) pass
    = fun prog ->
      ...
      let assign_clauses =
        List.of_seq
          begin
            let open OSeq in
            List.to_seq prog.pool
            >>= fun (i, _) ->
              indexed i
              >>= fun ii ->
                List.to_seq prog.locations
                >>= fun l ->
                  return
                    begin
                      let open Provable in
  ...
  let convertProvable
      : type a. ((MultipliedOrIndexed.t, IndexedItem.t) Atom.t Provable.program
      * ((MultipliedOrIndexed.t, IndexedItem.t) Atom.t -> bool), a, Prove.atom Provable.timed Formula.t Seq.t, a) pass
    = fun (prog, selection) ->
      Prove.convert prog selection,
      fun x -> x

  let accrueAssignConstraint
      : type a. (Prove.atom Provable.timed Formula.t Seq.t, a, Prove.atom Provable.timed Formula.t Seq.t, a) pass
    = fun proof_system ->
      ...
      let range (p : program) (r : RangeConstraint.t) : formula =
        ...
        let at_most =
          ...
          conj_seq
            begin
              let open OSeq in
              ordered_pairs_of_items
              >>= fun (i, i') ->
                ordered_pairs_of_locations
                >>= fun (l, l') ->
      ...
      (* XXX: There is probably a bug here: these are defined in terms
         ...
         indexed_items. *)
      in
      let capacities_formula = Seq.map (capacity P.the_program) (List.to_seq P.the_program.locations) in
      OSeq.flatten
      @@ OSeq.of_list
        [
          ranges_formula;
  ...
  let adjoinObservables
      : type a. (Prove.atom Provable.timed Formula.t Seq.t, a, Prove.atom Provable.timed Formula.t Seq.t * Prove.atom Provable.timed array, a) pass
    = fun formulas ->
      let observables =
  ...
  let convertToCnf
      : type a. (Prove.atom Provable.timed Formula.t Seq.t, a, TimedLiteral.t list Seq.t, a) pass
    = fun formulas ->
      formulas |> OSeq.flat_map TimedLiteral.cnf_seq,
      fun x -> x

  let observablesToLiterals
      : type a. (Prove.atom Provable.timed array, a, TimedLiteral.t array, a) pass
    = fun observables ->
      observables
  ...
  let compileMult
      : type a. (TimedLiteral.t list Seq.t, a, Mult.atom list list, a) pass
    = fun clauses ->
      Mult.compile (List.of_seq clauses),
      fun x -> x

  let indexObservables
      : type a. (TimedLiteral.t array, a, Mult.atom array, a) pass
    = fun observables ->
      observables
